	...
	pub fn new(config: &Config) -> Result<Self, Error> {
		...
		let (cache_ptr, dataset_ptr) = unsafe {
			let dataset_ptr = if M::has_dataset() {
				...
				if ptr.is_null() {
					return Err(Error::CacheAllocFailed)
				}
				Some(ptr)
			} else {
				None
			...
			if cache_ptr.is_null() {
				if M::has_dataset() {
					sys::randomx_release_dataset(dataset_ptr.unwrap());
				}
				return Err(Error::CacheAllocFailed)
			} 

			(cache_ptr, dataset_ptr)
		};

		Ok(Self { cache_ptr, dataset_ptr, _marker: PhantomData })
	}
...
mod tests {
	...
	fn should_create_light_vm() {
		let mut cache = LightCache::new(&Default::default()).unwrap();
		cache.init(&b"RandomX example key"[..]);
	...
	fn should_work_with_full_vm() {
		let mut light_cache = LightCache::new(&Default::default()).unwrap();
		light_cache.init(&b"RandomX example key"[..]);
		let mut light_vm = LightVM::new(Arc::new(light_cache), &Default::default());
		let hash = light_vm.calculate(&b"RandomX example input"[..]);
		let mut full_cache = FullCache::new(&Default::default()).unwrap();
		full_cache.init(&b"RandomX example key"[..]);
		let mut full_vm = FullVM::new(Arc::new(full_cache), &Default::default());
